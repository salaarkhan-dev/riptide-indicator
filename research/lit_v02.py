"""RIPTIDE LIT v0.2 ENGINE - the correctness rewrite, validated before Pine.

NOT a strategy and not bot code. This is the state machine, transcribed so it
can be run over real candles, because there is no Pine compiler here.

WHY v0.1 WAS REJECTED, AND WHAT THE MEASUREMENTS SAY ABOUT EACH CLAIM
────────────────────────────────────────────────────────────────────
v0.1 deviated from the documented pullback rule: it moved a correction's
confirmation level down every time the correction made a deeper low. That was
wrong to do, and the brief is right to strike it. But the deviation was a
PATCH OVER A DIFFERENT BUG, and this file is where that is demonstrated rather
than asserted.

The real defect: v0.1 ran a demand tracker and a supply tracker permanently
and in parallel, feeding one shared pivot stream. The documented algorithm is
TREND-RELATIVE - "assume the parent structure is bullish", ride the impulse,
detect the correction against it. Running a demand tracker through a sustained
downtrend means the correction it opens can never confirm, because price never
returns to the level: one correction stayed open for 9200 bars. v0.1 papered
over that by letting the level chase price down.

v0.2 instead orients the detector to the context's own direction and
re-initialises it on a trend flip, which is what the source describes. The
confirmation level is then FIXED at the correction-start candle exactly as
documented, and the starvation does not occur, because a bullish context only
tracks bullish corrections and a bullish correction in a bullish leg resolves.

THE FIXTURE. The reference indicator shows 23 taken inducements on ZEC 15m
over roughly 2000 bars, splitting 56.5 / 43.5 between reaching the BOS first
and the CHoCH first. That number, recorded in research/lit.py before the
premium subscription lapsed, is the only quantitative target available and is
what Main is calibrated against here. v0.1 Main produced 9.7 taken inducements
per 2000 bars - five times too few - while its Internal and Deep produced
hundreds, which is the clutter the brief describes.

WHAT THIS FILE CAN AND CANNOT PROVE. It can show event counts, phase
occupancy, level reachability, invariant violations and whether the three
depths are genuinely different structures. It cannot show that the Pine
compiles or that the drawing is legible. Those still need a chart.

    PYTHONPATH=. RIPTIDE_DEEP_CACHE=/tmp/deep python3 research/lit_v02.py
"""
...
class PBDet:
    ...
    def feed(self, i, h, l, o, c, hiBar, loBar):
        ...
        if self.state == PB_IMPULSE:
            started = (l < self.trkLo) if bull else (h > self.trkHi)
            if started:
                ...
                cont = (h > self.trkHi and l >= self.trkLo) if bull else \
                       (l < self.trkLo and h <= self.trkHi)
                if cont:
                    self.trkHi, self.trkLo = h, l
            return None
