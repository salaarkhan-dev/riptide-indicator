...
This file only produces signals. It does not score them, does not filter them
and has no opinion about them — `trendline_measure.py` does that. Keeping the
port in its own module means the thing being verified against the chart and the
thing being measured are the same object, and a change to one cannot silently
