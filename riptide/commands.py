...
LEGEND = (
    ...
    "• one chosen by the risk band — +112.7 R, <b>recovery 1.84</b>\n"
    "<b>Read the recovery column, not the drawdown.</b> Any rule that simply "
    "trades smaller shrinks return and drawdown together and is worth nothing "
    "— capping risk per event does exactly that, 1.14 to 1.13. Picking one "
    "moves the ratio, and it cuts time spent below a previous equity peak "
    "from 60% of the year to 32%.\n"
    "<b>Total R falls, and that is not the objection it looks like.</b> "
    "+112.7 against +180.7 would matter on an account that could hold twenty "
...
def status_text(db, state) -> str:
    ...
    # The grade now reads the POI and the HTF trend (SuperTrend AND
...
def stats_text(db) -> str:
    ...
    if any(b["setups"] for b in g.values()):
        body += (f"\n<b>by grade</b>  <i>({tf_word(POI_INTERVAL)} POI × "
                 f"HTF trend; "
