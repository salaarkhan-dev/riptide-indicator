...
LEGEND = (
    ...
    "ever seeing whether it was right. Two honest limits: the 1.2 and the 2.6 "
    "were chosen by looking at data, so some of the separation is selection; "
    "and the band was measured on <b>30m only</b>, so a 15m alert gets no "
    "verdict at all rather than one borrowed from a timeframe it was never "
    "tested on. Confirmed setups only — the same split on early signals is "
