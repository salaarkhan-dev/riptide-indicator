...
THE QUESTION, AND WHY IT IS A SEPARATE FILE. The study beside this one,
exhaustion.py, asked whether a completed count predicts anything and answered no — worse than
...
indicators/exhaustion/tests/test_exhaust.py asserts the two still agree.
...
forty rows in it is the thing that does not get read.
    ...
    PYTHONPATH=. RIPTIDE_DEEP_CACHE=/tmp/deep \
        python3 indicators/exhaustion/studies/exhaust_rate.py
...
from indicators.exhaustion.port.td import counts  # noqa: E402
